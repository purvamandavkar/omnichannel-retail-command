...
export async function importCSVToSQLite(csvData) {
    const database = await openDB();
    ...
    const count = await database.get('SELECT COUNT(*) as total FROM sales');
    console.log(`✅ Imported ${count.total} records into SQLite`);
    return count.total;
}
...
const Dashboard = () => {
  ...
  const handleRandomize = async () => {
    setRefreshing(true);
    try {
      const response = await axios.post(`${API_URL}/api/randomize`);
    ...
    return (
      <div className={`min-h-screen flex flex-col justify-center items-center bg-gradient-to-br ${currentTheme.gradient}`}>
        <div className={`${currentTheme.card} p-8 rounded-xl text-center max-w-md`}>
          ...
          <p className={`${currentTheme.textMuted} mb-4`}>Please check that the backend server is running.</p>
      ...
      <div className="container mx-auto px-4 py-6 max-w-7xl flex-1">
        ...
        <div className="border-b border-white/20 mb-6 overflow-x-auto">
          <div className="flex gap-2 flex-nowrap">
            <button onClick={() => setActiveTab('overview')} className={`px-6 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 whitespace-nowrap transition ${activeTab === 'overview' ? `bg-purple-500/30 text-purple-300 border-b-2 border-purple-500` : `${currentTheme.textMuted} hover:${currentTheme.text}`}`}>
              <LayoutDashboard className="w-4 h-4" /> Executive Overview
            </button>
            <button onClick={() => setActiveTab('sales')} className={`px-6 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 whitespace-nowrap transition ${activeTab === 'sales' ? `bg-purple-500/30 text-purple-300 border-b-2 border-purple-500` : `${currentTheme.textMuted} hover:${currentTheme.text}`}`}>
              <BarChart3 className="w-4 h-4" /> Sales Analytics
            </button>
            <button onClick={() => setActiveTab('inventory')} className={`px-6 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 whitespace-nowrap transition ${activeTab === 'inventory' ? `bg-purple-500/30 text-purple-300 border-b-2 border-purple-500` : `${currentTheme.textMuted} hover:${currentTheme.text}`}`}>
              <Package className="w-4 h-4" /> Inventory Optimization
            </button>
            <button onClick={() => setActiveTab('channels')} className={`px-6 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 whitespace-nowrap transition ${activeTab === 'channels' ? `bg-purple-500/30 text-purple-300 border-b-2 border-purple-500` : `${currentTheme.textMuted} hover:${currentTheme.text}`}`}>
              <PieChart className="w-4 h-4" /> Channel Performance
            </button>
            <button onClick={() => setActiveTab('customer')} className={`px-6 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 whitespace-nowrap transition ${activeTab === 'customer' ? `bg-purple-500/30 text-purple-300 border-b-2 border-purple-500` : `${currentTheme.textMuted} hover:${currentTheme.text}`}`}>
              <Users className="w-4 h-4" /> Customer 360°
            </button>
            <button onClick={() => setActiveTab('forecast')} className={`px-6 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 whitespace-nowrap transition ${activeTab === 'forecast' ? `bg-purple-500/30 text-purple-300 border-b-2 border-purple-500` : `${currentTheme.textMuted} hover:${currentTheme.text}`}`}>
        ...
        <div>
          {activeTab === 'overview' && (
            <>
              <div className="flex justify-end mb-4">
                <ExportReports data={kpis ? Object.entries(kpis).map(([key, value]) => ({ Metric: key, Value: value })) : []} title="Executive KPIs" filename="executive_kpis" />
              </div>
          ...
          {activeTab === 'sales' && salesAnalytics && (
            <>
              <div className="flex justify-end mb-4">
                <ExportReports data={salesAnalytics.topProducts || []} title="Top Products" filename="top_products" />
              </div>
          ...
          {activeTab === 'inventory' && inventoryOptimization && (
            <>
              <div className="flex justify-end mb-4">
                <ExportReports data={inventoryOptimization.quickReport || []} title="Inventory Report" filename="inventory_report" />
              </div>
          ...
          {activeTab === 'channels' && channelPerformance && (
            <>
              <div className="flex justify-end mb-4">
                <ExportReports data={channelPerformance.comparativePerformance || []} title="Channel Performance" filename="channel_performance" />
              </div>
